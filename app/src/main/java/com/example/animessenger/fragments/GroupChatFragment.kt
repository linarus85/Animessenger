...
//    }
    ...
    private fun initRecView() {
        ...
        refMessage = REF_DATABASE.child(NODE_GROUP)
    ...
    private fun initToolbar() {
        ...
        send.setOnClickListener {
            ...
            if (message.isEmpty()) {
                ...
                contact?.id?.let { it1 -> saveMainList(it1, TYPE_GROUP) }
                et_type_message.setText("")
    ...
    private fun sendMessageToGroup(message: String, id: String?, typeText: String, function: () -> Unit) {
       val refMessages = "$NODE_GROUP/$id/$NODE_MESSAGES"
        val messageKey = REF_DATABASE.child(refMessages).push().key
        val mapMessage = hashMapOf<String, Any>()
        mapMessage[CHILD_FROM_USER] = UID
        ...
        mapMessage[CHILD_TIME] = ServerValue.TIMESTAMP
        REF_DATABASE.child(refMessages).child(messageKey.toString())
